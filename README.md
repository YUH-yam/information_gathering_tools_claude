...
### 保存記事
- ステータス切替: 📌保存済 / 📥未判定 (inbox) / 🗑不要 (discarded)
- 検索（タイトル・概要・メモ）
- 重要度フィルタ
- カテゴリチップ絞り込み
- 重要度ブースト
- 各記事カードから「📌 保存する」「↩ 保存解除」「✕ 不要」「↩ 復元」「🗑 完全削除」を操作可能
...
主要ロジック（utils, classifier, articles, memos, reviews, exporter, streak, theme, status, delete, merge, RSS, dashboard）を 85 件カバー。
...
v1.4.0（テーマ切替「ライト/ダーク/自動」を追加。RSS取得の挙動を「自動保存」から「未判定 (inbox)」に変更し、ユーザーが今日見るで判断するまで保存記事に混入しない設計に。記事カードに「保存解除」「完全削除」「不要復元」を追加。保存記事画面に 📌保存済 / 📥未判定 / 🗑不要 の3タブを実装。テスト85件で全パス。SW CACHE_VERSION を v1.4.0 に更新）
